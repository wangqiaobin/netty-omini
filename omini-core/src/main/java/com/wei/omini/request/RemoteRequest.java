...
public class RemoteRequest {
    ...
    public <T extends RemoteParam> int request(String name, T data) {
    ...
    public <T extends RemoteParam> int receive(String name, String host, Integer port, T data) {
        Client client = getRemoteClient(name, host, port);
        Context context = ServerContextHandler.getInstance().removeContext(data.getReq());
        context.setState(2);
        context.setParam(data);
        client.getChannel().writeAndFlush(context);
        return 0;
